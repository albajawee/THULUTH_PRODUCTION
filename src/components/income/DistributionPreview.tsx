...
interface DistributionPreviewProps {
  ...
  total: number;
  /**
   * Required on purpose. This used to default to 'SAR', so the one caller that forgot to pass it
   * previewed every split in Riyals no matter which currency the user had saved.
   */
  currency: string;
}
  ...
  return (
    <Card>
      ...
      <CardContent>
        ...
        {/* Always equals `total`: the form refuses any amount the funds couldn't split exactly. */}
        <div className="mt-4 pt-3 border-t border-border/50 flex justify-between text-sm">
          ...
          <span className="font-semibold">{formatCurrency(distributed, currency)}</span>
        </div>
      </CardContent>
    </Card>
  );
